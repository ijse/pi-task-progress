...
- `taskTiming` is keyed by the decimal task ID. A timing record may contain `startedAt`, `completedAt`, or both.
- Times must be finite, non-negative integers. `completedAt` must not precede `startedAt`.
...
The publisher keeps an in-memory timing map per canonical project/session registry entry. Before publishing a new todo result or replaying the branch, it reconciles the last accepted task set with the next one at one captured `now()` time.

| Transition | Action |
| --- | --- |
| first seen as `in_progress` | Set `startedAt` if absent. |
| non-completed → `completed` | Set `startedAt` if absent and set `completedAt`. |
...
A branch replay reconstructs timing from ordered todo-result history where timestamps are available. When historical transition timing cannot be established, it does not invent dates: the task simply contributes no duration sample. On a running session, publisher state restored from its v2 snapshot preserves timing across extension hot reload.
...
6. **Current-task remaining** is `max(0, medianDuration - (observedAt - startedAt))` for the highlighted task. Its visual progress is elapsed/median, clamped to 95% until its todo state becomes completed.
...
- a concise next-task queue (up to two eligible pending tasks) with representative-duration estimates;
